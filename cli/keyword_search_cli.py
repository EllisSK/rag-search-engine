...
def main() -> None:
    ...
    build_parser = subparsers.add_parser(
        "build", help="Build the inverted index and save it to disk"
    )

    tf_parser = subparsers.add_parser(
        "tf", help="Get the term frequency of a single token"
    )
    ...
    bm25search_parser.add_argument(
        "limit", type=int, nargs="?", default=5, help="Limit number of results to"
    )
    bm25search_parser.add_argument(
        "k1", type=float, nargs="?", default=BM25_K1, help="Tunable BM25 K1 parameter"
    )
    ...
    match args.command:
        ...
        case "bm25search":
            ...
            for result in results:
                print(
                    f"{count}. ({result[0]}) {index.docmap[result[0]]['title']} - Score: {result[1]:.2f}"
                )
